...
if (isset($update['message'])) {
    ...
    if (preg_match('/^\/reply\s+([A-Z0-9]+)\s+(.+)$/is', $text, $matches)) {
        ...
        $stmt = $pdo->prepare('SELECT s.*, ac.id AS chat_id FROM shipments s LEFT JOIN agent_chats ac ON ac.shipment_id = s.id WHERE s.tracking_number = ? LIMIT 1');
        $stmt->execute([$trackingNumber]);
        $shipment = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($shipment) {
            $agent = get_agent_by_telegram_chat_id($chatId, $pdo);
            if (!$agent) {
                $agent = get_default_agent($pdo);
            }
            if (!$agent) {
                sendTelegramMessage($botToken, $chatId, "❌ No agent account linked to this Telegram user.");
                echo 'OK';
                exit;
            }

            $chat = ensure_agent_chat($pdo, $shipment, $agent, get_shipment_customer_details($shipment, $pdo));
            $savedMessage = save_agent_chat_message($chat, 'agent', $agent['name'], $replyMessage, [], $pdo);
