...
<form action="<?= $product ? '/admin/products/' . $product['id'] . '/edit' : '/admin/products/new' ?>" method="POST" enctype="multipart/form-data">
  <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;" class="product-form-grid">
    ...
    <div>
      ...
      <div class="card mb-4">
        <div class="card-header">
          <h3 class="card-title">Software Activation</h3>
        </div>
        <div class="card-body">
          <div class="form-group">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
              <input type="checkbox" name="has_software_activation" value="1" <?= ($product['has_software_activation'] ?? false) ? 'checked' : '' ?> onchange="toggleSoftwareFields()">
              <span>This product requires software activation</span>
            </label>
            <p style="font-size: 0.8rem; color: #64748b; margin-top: 4px;">Enable if this hardware product includes software that needs activation</p>
          </div>
          
          <div id="software-fields" style="<?= ($product['has_software_activation'] ?? false) ? '' : 'display: none;' ?>">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
              <div class="form-group">
                <label class="form-label">Software Price</label>
                <input type="number" name="software_price" class="form-control" 
                       value="<?= htmlspecialchars($product['software_price'] ?? '') ?>" 
                       step="0.01" min="0" placeholder="0.00">
              </div>
              <div class="form-group">
                <label class="form-label">Currency</label>
                <select name="software_currency" class="form-control">
                  <option value="EUR" <?= ($product['software_currency'] ?? 'EUR') === 'EUR' ? 'selected' : '' ?>>EUR</option>
                  <option value="USD" <?= ($product['software_currency'] ?? 'EUR') === 'USD' ? 'selected' : '' ?>>USD</option>
                </select>
              </div>
            </div>
            <p style="font-size: 0.8rem; color: #64748b; margin-top: 8px;">
              This is the price customers will pay for software activation. 
              If left empty, the software activation will be free.
            </p>
          </div>
        </div>
      </div>
...
function toggleSoftwareFields() {
  const checkbox = document.querySelector('input[name="has_software_activation"]');
  const fields = document.getElementById('software-fields');
