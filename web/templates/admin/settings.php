  ...
  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          <label class="form-label">EUR to USD Exchange Rate</label>
          <input type="number" name="eur_usd_rate" class="form-control" value="<?= htmlspecialchars($s['eur_usd_rate'] ?? '1.08') ?>" min="0" max="10" step="0.0001">
          <small style="color: #64748b;">1 EUR = X USD. Used for currency conversion in software activation.</small>
        </div>
        
        <div class="form-group">
          <label class="form-label">Last Updated</label>
          <div style="padding: 8px 12px; background: #f8fafc; border-radius: 4px; font-size: 0.9rem; color: #64748b;">
            <?= isset($s['eur_usd_updated']) ? date('M j, Y H:i', strtotime($s['eur_usd_updated'])) : 'Never' ?>
          </div>
          <small style="color: #64748b;">Exchange rates are automatically updated daily from API</small>
        </div>
      </div>
    </div>
  </div>
